const data = [{
    "id": 1,
    "name": "Eastern boa constrictor",
    "latest_chapter": 60,
    "image": "http://dummyimage.com/600x700.png/cc0000/ffffff"
}, {
    "id": 2,
    "name": "Collared lizard",
    "latest_chapter": 90,
    "image": "http://dummyimage.com/600x700.png/dddddd/000000"
}, {
    "id": 3,
    "name": "Kirk's dik dik",
    "latest_chapter": 90,
    "image": "http://dummyimage.com/600x700.png/ff4444/ffffff"
}, {
    "id": 4,
    "name": "Eastern box turtle",
    "latest_chapter": 65,
    "image": "http://dummyimage.com/600x700.png/dddddd/000000"
}, {
    "id": 5,
    "name": "Screamer, southern",
    "latest_chapter": 86,
    "image": "http://dummyimage.com/600x700.png/5fa2dd/ffffff"
}, {
    "id": 6,
    "name": "Southern white-crowned shrike",
    "latest_chapter": 71,
    "image": "http://dummyimage.com/600x700.png/dddddd/000000"
}, {
    "id": 7,
    "name": "White-winged dove",
    "latest_chapter": 90,
    "image": "http://dummyimage.com/600x700.png/cc0000/ffffff"
}, {
    "id": 8,
    "name": "Grey mouse lemur",
    "latest_chapter": 99,
    "image": "http://dummyimage.com/600x700.png/ff4444/ffffff"
}, {
    "id": 9,
    "name": "Skua, long-tailed",
    "latest_chapter": 84,
    "image": "http://dummyimage.com/600x700.png/cc0000/ffffff"
}, {
    "id": 10,
    "name": "Stork, white",
    "latest_chapter": 70,
    "image": "http://dummyimage.com/600x700.png/dddddd/000000"
}, {
    "id": 11,
    "name": "Stork, white",
    "latest_chapter": 68,
    "image": "http://dummyimage.com/600x700.png/cc0000/ffffff"
}, {
    "id": 12,
    "name": "Prehensile-tailed porcupine",
    "latest_chapter": 59,
    "image": "http://dummyimage.com/600x700.png/5fa2dd/ffffff"
}, {
    "id": 13,
    "name": "Golden brush-tailed possum",
    "latest_chapter": 73,
    "image": "http://dummyimage.com/600x700.png/ff4444/ffffff"
}, {
    "id": 14,
    "name": "Agile wallaby",
    "latest_chapter": 97,
    "image": "http://dummyimage.com/600x700.png/5fa2dd/ffffff"
}, {
    "id": 15,
    "name": "Seal, southern elephant",
    "latest_chapter": 68,
    "image": "http://dummyimage.com/600x700.png/cc0000/ffffff"
}, {
    "id": 16,
    "name": "Dove, little brown",
    "latest_chapter": 78,
    "image": "http://dummyimage.com/600x700.png/dddddd/000000"
}, {
    "id": 17,
    "name": "Small-toothed palm civet",
    "latest_chapter": 94,
    "image": "http://dummyimage.com/600x700.png/cc0000/ffffff"
}, {
    "id": 18,
    "name": "Racer, blue",
    "latest_chapter": 91,
    "image": "http://dummyimage.com/600x700.png/5fa2dd/ffffff"
}, {
    "id": 19,
    "name": "Wagtail, african pied",
    "latest_chapter": 64,
    "image": "http://dummyimage.com/600x700.png/dddddd/000000"
}, {
    "id": 20,
    "name": "Sheathbill, snowy",
    "latest_chapter": 70,
    "image": "http://dummyimage.com/600x700.png/dddddd/000000"
}, {
    "id": 21,
    "name": "Zorilla",
    "latest_chapter": 76,
    "image": "http://dummyimage.com/600x700.png/cc0000/ffffff"
}, {
    "id": 22,
    "name": "Southern sea lion",
    "latest_chapter": 50,
    "image": "http://dummyimage.com/600x700.png/cc0000/ffffff"
}, {
    "id": 23,
    "name": "Pelican, eastern white",
    "latest_chapter": 83,
    "image": "http://dummyimage.com/600x700.png/5fa2dd/ffffff"
}, {
    "id": 24,
    "name": "Vine snake (unidentified)",
    "latest_chapter": 73,
    "image": "http://dummyimage.com/600x700.png/dddddd/000000"
}, {
    "id": 25,
    "name": "Rock dove",
    "latest_chapter": 94,
    "image": "http://dummyimage.com/600x700.png/ff4444/ffffff"
}, {
    "id": 26,
    "name": "Opossum, american virginia",
    "latest_chapter": 96,
    "image": "http://dummyimage.com/600x700.png/cc0000/ffffff"
}, {
    "id": 27,
    "name": "Rat, white-faced tree",
    "latest_chapter": 63,
    "image": "http://dummyimage.com/600x700.png/ff4444/ffffff"
}, {
    "id": 28,
    "name": "Greater blue-eared starling",
    "latest_chapter": 69,
    "image": "http://dummyimage.com/600x700.png/ff4444/ffffff"
}, {
    "id": 29,
    "name": "Horned lark",
    "latest_chapter": 54,
    "image": "http://dummyimage.com/600x700.png/cc0000/ffffff"
}, {
    "id": 30,
    "name": "Fork-tailed drongo",
    "latest_chapter": 89,
    "image": "http://dummyimage.com/600x700.png/ff4444/ffffff"
}, {
    "id": 31,
    "name": "Sociable weaver",
    "latest_chapter": 79,
    "image": "http://dummyimage.com/600x700.png/ff4444/ffffff"
}, {
    "id": 32,
    "name": "African wild dog",
    "latest_chapter": 69,
    "image": "http://dummyimage.com/600x700.png/5fa2dd/ffffff"
}, {
    "id": 33,
    "name": "Pallas's fish eagle",
    "latest_chapter": 50,
    "image": "http://dummyimage.com/600x700.png/ff4444/ffffff"
}, {
    "id": 34,
    "name": "Crane, brolga",
    "latest_chapter": 62,
    "image": "http://dummyimage.com/600x700.png/ff4444/ffffff"
}, {
    "id": 35,
    "name": "Bear, grizzly",
    "latest_chapter": 56,
    "image": "http://dummyimage.com/600x700.png/dddddd/000000"
}, {
    "id": 36,
    "name": "Yellow-brown sungazer",
    "latest_chapter": 81,
    "image": "http://dummyimage.com/600x700.png/ff4444/ffffff"
}, {
    "id": 37,
    "name": "Lion, australian sea",
    "latest_chapter": 58,
    "image": "http://dummyimage.com/600x700.png/ff4444/ffffff"
}, {
    "id": 38,
    "name": "Sheathbill, snowy",
    "latest_chapter": 78,
    "image": "http://dummyimage.com/600x700.png/ff4444/ffffff"
}, {
    "id": 39,
    "name": "Giraffe",
    "latest_chapter": 75,
    "image": "http://dummyimage.com/600x700.png/ff4444/ffffff"
}, {
    "id": 40,
    "name": "Wombat, southern hairy-nosed",
    "latest_chapter": 69,
    "image": "http://dummyimage.com/600x700.png/dddddd/000000"
}, {
    "id": 41,
    "name": "Blue and gold macaw",
    "latest_chapter": 68,
    "image": "http://dummyimage.com/600x700.png/cc0000/ffffff"
}, {
    "id": 42,
    "name": "Bear, grizzly",
    "latest_chapter": 65,
    "image": "http://dummyimage.com/600x700.png/dddddd/000000"
}, {
    "id": 43,
    "name": "Otter, oriental short-clawed",
    "latest_chapter": 98,
    "image": "http://dummyimage.com/600x700.png/dddddd/000000"
}, {
    "id": 44,
    "name": "Steenbok",
    "latest_chapter": 76,
    "image": "http://dummyimage.com/600x700.png/cc0000/ffffff"
}, {
    "id": 45,
    "name": "Common zebra",
    "latest_chapter": 89,
    "image": "http://dummyimage.com/600x700.png/ff4444/ffffff"
}, {
    "id": 46,
    "name": "Swainson's francolin",
    "latest_chapter": 76,
    "image": "http://dummyimage.com/600x700.png/5fa2dd/ffffff"
}, {
    "id": 47,
    "name": "Deer, barasingha",
    "latest_chapter": 83,
    "image": "http://dummyimage.com/600x700.png/cc0000/ffffff"
}, {
    "id": 48,
    "name": "Gull, swallow-tail",
    "latest_chapter": 55,
    "image": "http://dummyimage.com/600x700.png/cc0000/ffffff"
}, {
    "id": 49,
    "name": "Macaw, blue and yellow",
    "latest_chapter": 93,
    "image": "http://dummyimage.com/600x700.png/cc0000/ffffff"
}, {
    "id": 50,
    "name": "Asian elephant",
    "latest_chapter": 84,
    "image": "http://dummyimage.com/600x700.png/cc0000/ffffff"
}];

const $bookList = document.getElementById('wibu-book-list');

// let result = "";
// for (const item of data) {
//     result += `
//          <div class="col-3 mb-3">
//              <div class="card">
//                  <img class="card-img-top"
//                      src="${item.image}"
//                      alt="">
//                  <div class="card-body">
//                      <b class="card-title">${item.name}</b>
//                      <p class="card-text">Chapter ${item.latest_chapter}</p>
//                         <a href="#" class="btn btn-primary btn-sm">Đọc truyện</a>
//                      <a href="#" class="btn btn-danger btn-sm">Yêu thích</a>
//                  </div>
//              </div>
//          </div>`;
// }
// console.log(result);
// $bookList.innerHTML = result;

$bookList.innerHTML = data.map(({ name, latest_chapter, image }) => {
    return `
    <div class="col-3 mb-3">
        <div class="card">
            <img class="card-img-top"
                src="${image}"
                alt="">
            <div class="card-body">
                <b class="card-title">${name}</b>
                <p class="card-text">Chapter ${latest_chapter}</p>

                <a href="#" class="btn btn-primary btn-sm">Đọc truyện</a>
                <a href="#" class="btn btn-danger btn-sm">Yêu thích</a>
            </div>
        </div>
    </div>`;
}).join('');


    //c1 biến 1 mảng gồm các object, r với mảng đó dùng phương thức join thành chuỗi HTML
    // $bookList.innerHTML = data.map(({name, latest_chapter, image}) => {
    //     return ``;
    // })

    // c2:dùng for đổ dữ liêu
    // tạo một chuỗi rỗng . sau đó dùng for lấy dữ liệu nguồn từ mảng object đổ 
    //dữ liệu vào chuỗi rỗng ta thành 1 mảng dữ liệu với html
